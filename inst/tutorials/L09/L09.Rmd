...
quiz(
  ...
  question("2 关于 `stringr` 包内置数据集 `words` 说法正确的有：",
    ...
    answer("`words` 数据集中单词开头字母和结尾字母相同的单词共有32个。", 
           correct = FALSE, "事实上，`words` 数据集中单词开头字母和结尾字母相同的单词共有36个，而非32个。"),
    answer("`words` 数据集中存在含有 `z`、`e`、`n`、`g` 全部4个字母的单词。&#x1F601;", 
           correct = TRUE),
    answer("`words` 数据中以 `k` 字母开头的单词比 `k` 为第3个字母的单词多。",
           correct = TRUE), 
    correct = "Bingo！", 
    try_again = "错误！请利用 `stringr` 包中的相关函数（如 `str_subset()`、`str_detect()`、`str_ends()` 等）及恰当的正则表达式对上述说法进行一一测试。",
  ...
  question("3 认真想想，下面哪些 `stringr` 包中的函数能直接用在 `dplyr::mutate()` 函数中生成新变量？",
    ...
    try_again = "错误！这是到多~~选题，要点在于 `mutate()` 需要的是能返回 **与输入向量等长输出结果** 的向量化函数。请查看 `stringr` 包的 cheatsheet 的图示或相应 `str_*()` 函数的帮助文档，特别是返回值 `Value` 部分。",
  ...
  question("4 下面哪个 `forcats` 包中的函数是用来调整因子水平？",
           type = "multiple", 
    answer('`fct_relevel()`', 
    ...
    post_message = "`fct_recode()` 的变体 `fct_collapse()` 函数也可用来（更便捷地）调整因子水平；此外，`fct_lump_prop()`、`fct_lump_n()`、`fct_lump_lowfreq()`、`fct_lump_min()`、`fct_other()` 等几个调整因子水平的函数也值得你了解并拥有。", 
    allow_retry = TRUE,
    submit_button = "提交答案",
    try_again_button = "重新回答"
  ),
  
  question("5 日期-时间的处理从来都不是个简单的问题，相关 R 包往往都需要将时间存储为（相对某个基点时间算起的）数值。那么请问：`lubridate::as_date(\"2020-12-25\")` 和 `lubridate::as_datetime(\"2020-12-25\")` 在内部分别存储为：<span style='color: red'>（&#x1F4A3; 注意此题答错不允许 Try Again &#x1F4A3;）</span>",
...
请同学们点击下面的 [Generate] 按钮在方框中生成答案的 hash 码，然后点击方框右上角上的复制按钮，并将得到的hash码直接粘贴到 [坚果云 Excel 表格收集表 QFwR-L09](https://workspace.jianguoyun.com/inbox/collect/0fb5400a1b4e43c4a508c688ba9075ed/submit) 中。
...
接下来你可在 `RStudio` 的 Console 中键入如下命令调出本讲练习的 `RMarkdown` 文档（`L09-lab.Rmd`），根据提示完成上机实操练习。
